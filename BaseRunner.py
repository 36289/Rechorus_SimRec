...
class BaseRunner(object):
    ...
    def train(self, data_dict: Dict[str, BaseModel.Dataset]):
        ...
        logging.info(os.linesep + f"Best Iter(dev)={best_epoch + 1}\t dev=({utils.format_metric(dev_results[best_epoch])}) [{self.time[1] - self.time[0]:<.1f} s] ")
        model.load_model()
    ...
    def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
        ...
        for batch in tqdm(dl, leave=False, desc=f'Epoch {epoch:<3}', ncols=100, mininterval=1):
            ...
            # 添加检查以确保模型输出包含预测信息
            if out_dict is None or 'prediction' not in out_dict:
                raise ValueError(f"Model forward pass did not return a valid prediction dictionary in epoch {epoch}. Got {out_dict} instead.")
    ...
    def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
        ...
        for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
            ...
            # 检查模型输出
            if out_dict is None or 'prediction' not in out_dict:
                raise ValueError(f"Model forward pass did not return a valid prediction dictionary during prediction. Got {out_dict} instead.")
